...
class XmlEventDefinition extends XmlProductionDefinition {
  ...
  Parser characterData() => super
      .characterData()
      .token()
      .map((each) => XmlTextEvent(each.value, XmlEventSourceRange(each.start, each.stop)));
  ...
  Parser startElement() => XmlToken.openElement
      ...
      .token()
      .map((each) => XmlStartElementEvent(each.value[1], each.value[2].cast<XmlEventAttribute>(),
          each.value[4] == XmlToken.closeEndElement, XmlEventSourceRange(each.start, each.stop)));
  ...
  Parser comment() => super
      .comment()
      .token()
      .map((each) => XmlCommentEvent(each.value[1], XmlEventSourceRange(each.start, each.stop)));

  @override
  Parser cdata() => super
      .cdata()
      .token()
      .map((each) => XmlCDATAEvent(each.value[1], XmlEventSourceRange(each.start, each.stop)));

  @override
  Parser declaration() => super.declaration().token().map((each) => XmlDeclarationEvent(
      each.value[1].cast<XmlEventAttribute>(), XmlEventSourceRange(each.start, each.stop)));

  @override
  Parser processing() => super.processing().token().map(
      (each) => XmlProcessingEvent(each.value[1], each.value[2], XmlEventSourceRange(each.start, each.stop)));

  @override
  Parser doctype() => super
      .doctype()
      .token()
      .map((each) => XmlDoctypeEvent(each.value[2], XmlEventSourceRange(each.start, each.stop)));
}
